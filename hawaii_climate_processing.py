#!/usr/bin/env python3
"""
Hawaii Climate Normals Processing Pipeline - Multiprocessing Version

Specialized script for processing temperature variables (tas, tasmax, tasmin) for Hawaii region.
...
# Configuration for Hawaii climate processing
INPUT_DATA_DIR = "/media/mihiarc/RPA1TB/data/NorESM2-LM"
OUTPUT_BASE_DIR = "output/hawaii_normals"
...
# Progress tracking
PROGRESS_STATUS_FILE = "hawaii_processing_progress.json"
PROGRESS_LOG_FILE = "hawaii_processing_progress.log"
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('hawaii_climate_processing.log')
        ]
    )
...
def compute_climate_normal_safe(data_arrays: List, years: List[int], target_year: int) -> Optional[xr.DataArray]:
    ...
    try:
        ...
        mean_data.attrs.update({
            ...
            'processing_method': 'multiprocessing_safe_hawaii'
        })
        ...
        for target_year in target_years:
            try:
                ...
                climate_normal.attrs.update({
                    ...
                    'processing': 'Hawaii multiprocessing pipeline',
...
def main():
    """Main processing function for Hawaii climate normals."""
    start_time = time.time()
    logger = setup_logging()
    
    print("🌺 Starting Hawaii Climate Normals Processing Pipeline")
    print(f"Variables: {', '.join(VARIABLES)}")
    print(f"Region: {REGION} (Hawaii)")
    ...
    print(f"\n🎉 Hawaii Climate Processing Complete!")
    ...
    with open('hawaii_processing_summary.json', 'w') as f:
        json.dump(summary, f, indent=2)
    
    print(f"Summary saved to: hawaii_processing_summary.json")
