#!/usr/bin/env python3
"""
PRVI (Puerto Rico & Virgin Islands) Climate Normals Processing Pipeline - Multiprocessing Version

Specialized script for processing temperature variables (tas, tasmax, tasmin) for PRVI region.
...
# Configuration for PRVI climate processing
INPUT_DATA_DIR = "/media/mihiarc/RPA1TB/data/NorESM2-LM"
OUTPUT_BASE_DIR = "output/prvi_normals"
VARIABLES = ['tas', 'tasmax', 'tasmin']  # All temperature variables
REGION = 'PRVI'  # Puerto Rico and Virgin Islands
...
# Progress tracking
PROGRESS_STATUS_FILE = "prvi_processing_progress.json"
PROGRESS_LOG_FILE = "prvi_processing_progress.log"
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('prvi_climate_processing.log')
        ]
    )
...
def compute_climate_normal_safe(data_arrays: List, years: List[int], target_year: int) -> Optional[xr.DataArray]:
    ...
    try:
        ...
        mean_data.attrs.update({
            ...
            'processing_method': 'multiprocessing_safe_prvi'
        })
        ...
        for target_year in target_years:
            try:
                ...
                climate_normal.attrs.update({
                    ...
                    'processing': 'PRVI multiprocessing pipeline',
...
def main():
    """Main processing function for PRVI climate normals."""
    start_time = time.time()
    logger = setup_logging()
    
    print("🏝️  Starting PRVI Climate Normals Processing Pipeline")
    print("📍 Puerto Rico & U.S. Virgin Islands")
    print(f"Variables: {', '.join(VARIABLES)}")
    print(f"Region: {REGION}")
    print(f"Cores per variable: {CORES_PER_VARIABLE}")
    print(f"Total cores: {len(VARIABLES) * CORES_PER_VARIABLE}")
    print(f"Region bounds: 292.03°-295.49°E, 17.62°-18.57°N")
    ...
    print(f"\n🎉 PRVI Climate Processing Complete!")
    ...
    summary = {
        ...
        'region': REGION,
        'region_name': 'Puerto Rico and U.S. Virgin Islands',
        'variables': VARIABLES,
        'total_years_processed': total_years_processed,
        'variable_results': variable_results
    }
    
    with open('prvi_processing_summary.json', 'w') as f:
        json.dump(summary, f, indent=2)
    
    print(f"Summary saved to: prvi_processing_summary.json")
