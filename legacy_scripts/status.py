...
def count_files_by_category():
    ...
    base_path = Path("output/rolling_30year_climate_normals")
    ...
    for variable in variables:
        for period in periods:
            period_path = base_path / variable / period
...
def quick_status():
    ...
    stats, total_files = count_files_by_category()
    if stats is None:
        print("❌ Output directory not found - processing may not have started")
        return
    
    targets, total_target = calculate_targets()
