#!/usr/bin/env python3
"""
Climate Data Processing Library

A comprehensive library for climate data processing that combines climate calculations 
and regional operations for processing climate data with crash resistance 
...
- I/O utilities for NetCDF climate data
- Time coordinate handling and climatology calculations

Core Functions:
- process_climate_data_workflow: Complete processing workflow
- compute_climate_normal: Climate normal calculations

Use run_climate_means.py to execute processing workflows.
"""
...
# Import regional functionality
from src.regions import REGION_BOUNDS, extract_region, validate_region_bounds

# Import I/O functionality
from src.io_util import open_dataset_safely, NorESM2FileHandler, save_climate_result, SAFE_CHUNKS

# Import time handling functionality
from src.time_util import (
...
# =============================================================================
# CLIMATE CALCULATIONS AND PROCESSING
# =============================================================================
...
# =============================================================================
# PROCESSING FUNCTIONS (CORE FUNCTIONALITY)
# =============================================================================

def process_file(file_path: str, variable_name: str, region_key: str, file_handler) -> Tuple[Optional[int], Optional[np.ndarray]]:
    """
...
def process_period_region(period_info: Tuple[int, int, int, str], variable_name: str, 
                         file_handler, region_key: str) -> Optional[xr.DataArray]:
    """
    Process a climate period for a specific region and variable using sequential processing.
    ...
    try:
        ...
        for i, file_path in enumerate(files):
            ...
            year, daily_clim = process_file(file_path, variable_name, region_key, file_handler)
...
def process_climate_data_workflow(data_directory: str, output_directory: str, 
                                variables: List[str], regions: List[str], 
                                scenarios: List[str], config: Dict[str, Any]) -> None:
    """
    Complete workflow for processing climate data using sequential processing.
    
    Args:
        data_directory: Directory containing input NetCDF files
        ...
        config: Configuration dictionary (used for logging/monitoring)
    """
    ...
    try:
        ...
        data_availability_detailed = file_handler.validate_data_availability()
            ...
            for variable in variables:
                ...
                for region_key in regions:
                    ...
                    for period_info in periods:
                        ...
                        result = process_period_region(
                            period_info, variable, file_handler, region_key
                        )
                        
                        if result is not None:
                            # Save result
                            success = save_climate_result(result, output_path, variable, region_key, period_name)
                            if success:
