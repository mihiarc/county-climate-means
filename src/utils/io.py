...
from .time_handling import extract_year_from_filename, get_time_decoding_params, try_time_engines
...
SAFE_CHUNKS = {
    ...
    'lat': 100,     # Larger spatial chunks to reduce overhead
    'lon': 100      # Larger spatial chunks to reduce overhead
}
...
def open_dataset_safely(file_path: str) -> Optional[xr.Dataset]:
    ...
    try:
        ...
        error_msg = str(e)
        logger.error(f"Failed to open {Path(file_path).name}: {error_msg}")
        return None
    ...
    def get_files_for_period(self, variable_name: str, scenario: str, start_year: int, end_year: int) -> List[str]:
        ...
        try:
            ...
            for file_path in scenario_dir.glob(pattern):
                ...
                year = self.extract_year_from_filename(file_str)
